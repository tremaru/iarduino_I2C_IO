...
void	iarduino_I2C_IO::pinMode				(uint8_t i,	uint8_t j){												//
			...
			EXP_uint_DATA = selI2C->readByte(EXP_uint_ADDRESS, EXP_uint_COMMAND);									//	Читаем байт данных из регистра EXP_uint_COMMAND модуля с адресом EXP_uint_ADDRESS в переменную EXP_uint_DATA
			...
			selI2C->writeByte(EXP_uint_ADDRESS, EXP_uint_COMMAND, EXP_uint_DATA);									//	Записываем данные из переменной EXP_uint_DATA в регистр EXP_uint_COMMAND модуля с адресом EXP_uint_ADDRESS
}																													//
																													//
